...
RSpec.describe QuestionsController, type: :controller do
  ...
  describe 'DELETE #destroy_file_attachment' do
    ...
    #    body: question.body,
    #    files: [ 
    #      Rack::Test::UploadedFile.new("#{Rails.root}/spec/rails_helper.rb", "text/plain"),
    #      Rack::Test::UploadedFile.new("#{Rails.root}/spec/spec_helper.rb", "text/plain")
    #    ]
    #  }, format: :js } 
      
    #  sign_out(question.user)
    #end
    ...
    context 'when user authenticated' do
      context 'and user is author' do
        ...
        it 'deletes the question attachment' do
          expect { delete :destroy_file_attachment, params: { id: question.files.first.signed_id }, format: :js }.to change(ActiveStorage::Attachment, :count).by(-1)
        end

        it 'renders destroy_file_attachment view' do
          delete :destroy_file_attachment, params: { id: question.files.first.signed_id }, format: :js
      ...
      context 'and user is not and author' do
        ...
        it 'not deletes the question attachment' do
          expect { delete :destroy_file_attachment, params: { id: question.files.first.signed_id }, format: :js }.not_to change(ActiveStorage::Attachment, :count)
        end
    ...
    context 'when user is not authenticated' do
      ...
      it 'does not delete the question attachment' do
        expect { delete :destroy_file_attachment, params: { id: question.files.first.signed_id }, format: :js }.not_to change(ActiveStorage::Attachment, :count)
      end
    end
  end
